...
describe("final redesigned shell accessibility and no-overlap contracts", () => {
  ...
  it("keeps no-overlap responsive layout constraints in the shared shell stylesheet", () => {
    ...
    expect(themeCss).toMatch(
      /\.crosslog-pane-workspace__content\s*\{[^}]*position:\s*relative;[^}]*display:\s*flex;[^}]*min-inline-size:\s*100%;/s,
    );
    expect(themeCss).toMatch(
      /\.crosslog-pane-reorder-outline\s*\{[^}]*position:\s*absolute;[^}]*z-index:\s*7;[^}]*border:\s*2px dashed var\(--crosslog-accent\);[^}]*pointer-events:\s*none;/s,
    );
    expect(themeCss).toMatch(
